...
parser.add_argument('-max_predictions_per_seq', type=int, default=20,
                    help="max number of predictions in oneseq")
...
def make_translation_data(src_file, tgt_file, src_dicts, tgt_dicts, max_src_length=64, max_tgt_length=64,
    ...
    while True:
        ...
        # 在文本中间的位置，如果src 不是空，tgt为空，这个时候tline 并不会是 "", 而是"\n"， 只有最后一行之后为空才是"",才会进入到分支里面去
        if sline == "" or tline == "":
        ...
        if len(src_words) <= max_src_length \
            ...
            if add_bos:
                tgt += [tgt_dicts.convertToIdx(tgt_words,
                                               ...
                                               onmt.Constants.TGT_EOS_WORD, type=data_type)]
                ...
                tgt += [tgt_dicts.convertToIdx(tgt_words,
                                               onmt.Constants.TGT_UNK_WORD,
                                               None,
                                               onmt.Constants.TGT_BOS_WORD, type=data_type)]
...
def main():
    ...
    if opt.lm:
        ...
        for seq in train['src']:
            n_pred = min(opt.max_predictions_per_seq, int(round(len(seq) * 0.015)))
            cand_maked_pos = [i for i, idx in enumerate(list(seq))
                              if idx != onmt.Constants.BERT_BOS and idx != onmt.Constants.BERT_EOS]
            shuffle(cand_maked_pos)
            masked_tokens, masked_pos = [], []
            for pos in cand_maked_pos[:n_pred]:
                masked_pos.append(pos)
                masked_tokens.append(seq[pos])
                index = randint(0, len(dicts['src'].frequencies) - 1)  # random index in vocabulary
                seq[pos] = index  # replace
