...
class ScalarMix(torch.nn.Module):
    ...
    def forward(self, tensors: List[torch.Tensor], mask: torch.BoolTensor = None) -> torch.Tensor:
        ...
        parameters = torch.cat([parameter for parameter in self.scalar_parameters])
        params = parameters[0:len(tensors)]
        normed_weights = torch.nn.functional.softmax(params, dim=0)
        ...
        if not self.do_layer_norm:
            ...
            weighted_context = self.gamma * sum(pieces)
            return weighted_context.transpose(0, 1)