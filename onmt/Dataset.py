...
class Batch(object):
    ...
    def __init__(self, src_data, tgt_data=None,
        ...
        if tgt_data is not None:
            target_full, self.tgt_lengths = self.collate(tgt_data,
                                                         align_right=self.tgt_align_right)
            ...
            self.tensors['tgt_mask'] = self.tensors['target_output'].ne(onmt.Constants.TGT_PAD)
    ...
    def collate(self, data, align_right=False, type="text", augmenter=None, lang="tgt"):
        """
...
class Dataset(object):
    ...
    def next(self, curriculum=False, reset=True, split_sizes=1):
        ...
        # 调用的是__get__item（self，index）的方法，self（trainer实例对象） 会传给 __get__item 的self， batch_index 传给index
